...
FACTORY_PRODUCTS = {
    "clothes": {
        "name": "👕 لباس هاپویی",
        "price": 50,
        "db_field": "inventory_clothes",
    },
    "food": {
        "name": "🦴 غذای ویژه",
        "price": 100,
        "db_field": "inventory_food",
    },
    "toy": {"name": "🎾 توپ بازی", "price": 30, "db_field": "inventory_toy"},
    "house": {
        "name": "🏠 لانه شیک",
        "price": 1000,
        "db_field": "inventory_house",
    },
}
...
CONTRABAND_PRODUCTS = {
    "cig": {
        ...
        "cost": 500,
        "profit": 1500,
        "db_field": "inventory_cig",
    },
    "diamond": {
        "name": "💎 الماس سیاه",
        "cost": 2000,
        "profit": 6000,
        "db_field": "inventory_diamond",
    },
    "gold": {
        "name": "🪙 شمش طلا",
        "cost": 5000,
        "profit": 15000,
        "db_field": "inventory_gold",
    },
    "car": {
        "name": "🏎 خودروی قاچاق",
        "cost": 10000,
        "profit": 35000,
        "db_field": "inventory_car",
    },
}
...
def _ensure_user_dict(user, user_id: int) -> dict:
    ...
    bank = (
        db.get_user_field(user_id, "bank_balance")
        if hasattr(db, "get_user_field")
        else 0
    )
    points = (
        db.get_user_field(user_id, "points")
        if hasattr(db, "get_user_field")
        else 0
    )
...
async def handle_bank_callback(
    ...
    if data == "bank_deposit":
        ...
        await query.message.reply_text(
            "💵 لطفاً مبلغی که می‌خواهید به بانک واریز کنید را وارد کنید (یا بنویسید `همه`):"
        )

    elif data == "bank_withdraw":
        context.user_data["state"] = "WAITING_FOR_BANK_WITHDRAW"
        await query.message.reply_text(
            "🏧 لطفاً مبلغی که می‌خواهید از بانک برداشت کنید را وارد کنید (یا بنویسید `همه`):"
        )

    elif data == "bank_claim_profit":
        if hasattr(db, "claim_daily_interest"):
            success, message, profit = db.claim_daily_interest(
                user_id, interest_rate=0.03
            )
            await query.message.reply_text(message)
        else:
            await query.message.reply_text(
                "❌ سیستم سود دیتابیس فعال نیست."
            )

    elif data == "bank_change_account":
        context.user_data["state"] = "WAITING_FOR_NEW_ACCOUNT_NUM"
        await query.message.reply_text(
            "💳 لطفاً شماره حساب جدید خود را وارد کنید:"
        )
...
async def show_factory(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    in_jail = (
        db.is_in_jail(user_id)[0] if hasattr(db, "is_in_jail") else False
    )
    if in_jail:
        await update.message.reply_text(
            "🔒 شما در زندان هستید و به کارخانه دسترسی ندارید!"
        )
        return
    ...
    for code, item in FACTORY_PRODUCTS.items():
        ...
        keyboard.append([
            InlineKeyboardButton(
                f"{item['name']} - {price_str}",
                callback_data=f"buy_fac_{code}:{user_id}",
            )
        ])

    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text(
        "🏭 **به کارخانه خوش آمدید!**\nمحصول مورد نظر برای خرید را انتخاب کنید:",
        reply_markup=reply_markup,
        parse_mode="Markdown",
    )


async def factory_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    query = update.callback_query
    await query.answer()
...
async def show_contraband(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    user_id = update.effective_user.id

    in_jail, jail_until = (
        db.is_in_jail(user_id) if hasattr(db, "is_in_jail") else (False, None)
    )
    if in_jail:
        keyboard = [[
            InlineKeyboardButton(
                "🔓 آزادی فوری (۲۰,۰۰۰ میو/هاپ)",
                callback_data=f"pay_bail:{user_id}",
            )
        ]]
    ...
    for code, item in CONTRABAND_PRODUCTS.items():
        ...
        keyboard.append([
            InlineKeyboardButton(
                f"{item['name']} (هزینه: {cost_str} | سود: {profit_str})",
    ...
    keyboard.append([
        InlineKeyboardButton(
            "🚀 شروع عملیات قاچاق",
            callback_data=f"start_smuggling:{user_id}",
        )
    ])
...
async def handle_smuggle_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    query = update.callback_query
    user_id = query.from_user.id
    ...
    if data == "pay_bail":
        ...
        if points < 20000:
            await query.message.reply_text(
                "❌ شما ۲۰,۰۰۰ موجودی برای وثیقه ندارید!"
            )
            return
        ...
        await query.message.reply_text(
            "🔓 شما با پرداخت ۲۰,۰۰۰ وثیقه از زندان آزاد شدید!"
        )
        return
    ...
    if data.startswith("select_contra_"):
        ...
        await query.message.reply_text(
            f"تعداد مورد نظر برای **{CONTRABAND_PRODUCTS[code]['name']}** را وارد کنید:"
        )
        return
    ...
    if data == "start_smuggling":
        ...
        if context.user_data.get("is_smuggling_active", False):
            await query.message.reply_text(
                "⏳ شما از قبل یک قاچاق در انتظار دارید! صبر کنید تا محموله قبلی برسد."
            )
            return

        total_cost = sum(
            CONTRABAND_PRODUCTS[code]["cost"] * qty
        ...
        if points < total_cost:
            await query.message.reply_text(
                f"❌ موجودی کافی نیست! هزینه کل: {format_balance(total_cost)}"
            )
            return
        ...
        await query.message.reply_text(
            "🚚 **عملیات قاچاق آغاز شد!**\nمحموله ارسال شد. نتیجه تا ۳۰ دقیقه دیگر مشخص می‌شود..."
        )

        asyncio.create_task(
            process_smuggling_result(context, user_id, total_cost, cart)
        )
...
async def process_smuggling_result(context, user_id, total_cost, cart):
    ...
    if is_busted:
        ...
        total_profit = sum(
            CONTRABAND_PRODUCTS[code]["profit"] * qty
            for code, qty in cart.items()
        )
...
async def start_gamble(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 2:
        await update.message.reply_text(
            "❌ فرمت صحیح: `قمار [مبلغ] [تعداد نفرات]`\nمثال: `قمار 100 3`",
            parse_mode="Markdown",
        )
        return
    ...
    try:
        ...
        await update.message.reply_text(
            "❌ مبلغ و تعداد افراد باید عدد معتبر باشند."
        )
        return

    if amount <= 0 or max_players < 2:
        await update.message.reply_text(
            "❌ حداقل مبلغ ۱ و حداقل تعداد شرکت‌کنندگان ۲ نفر است."
        )
        return
    ...
    if user_points < amount:
        await update.message.reply_text(
            "❌ کافی نمی‌باشد تعداد هاپ پوینت‌های شما."
        )
        return
    ...
    keyboard = InlineKeyboardMarkup([[
        InlineKeyboardButton(
            "🎲 شرکت در قمار", callback_data=f"join_gamble:{gamble_id}"
        )
    ]])
...
async def join_gamble_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    query = update.callback_query
    user = query.from_user
    ...
    if gamble_id not in active_gambles:
        await query.answer(
            "❌ این قمار پایان یافته یا منقضی شده است.", show_alert=True
        )
        return
    ...
    if user.id in player_ids:
        await query.answer(
            "❌ شما قبلاً در این قمار شرکت کرده‌اید!", show_alert=True
        )
        return
    ...
    if user_points < gamble["amount"]:
        await query.answer(
            "❌ برای شرکت در این قمار موجودی کافی ندارید.", show_alert=True
        )
        return
    ...
    if current_count >= gamble["max_players"]:
        ...
        await query.edit_message_text(
            updated_text, reply_markup=keyboard, parse_mode="Markdown"
        )
...
async def handle_factory_and_smuggle_text(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    ...
    if state == "WAITING_FOR_BANK_DEPOSIT":
        ...
        if text in ["همه", "all"]:
            ...
            await update.message.reply_text(
                "❌ لطفاً یک عدد معتبر یا کلمه `همه` را وارد کنید."
            )
            return True

        if amount <= 0:
            await update.message.reply_text(
                "❌ مقدار واریز باید بیشتر از صفر باشد."
            )
            return True
        ...
        if points < amount:
            ...
            await update.message.reply_text(
                f"✅ مبلغ **{format_balance(amount)}** هاپ به بانک واریز شد."
            )
    ...
    if state == "WAITING_FOR_BANK_WITHDRAW":
        ...
        if amount <= 0:
            await update.message.reply_text(
                "❌ مقدار برداشت باید بیشتر از صفر باشد."
            )
            return True

        if bank_bal < amount:
            await update.message.reply_text(
                f"❌ موجودی بانک شما کافی نیست!\n🏦 موجودی بانک: {format_balance(bank_bal)}"
            )
        else:
            db.update_field(user_id, "bank_balance", -amount, relative=True)
            db.update_field(user_id, "points", amount, relative=True)
            await update.message.reply_text(
                f"✅ مبلغ **{format_balance(amount)}** هاپ از بانک برداشت شد."
            )
    ...
    if state == "WAITING_FOR_NEW_ACCOUNT_NUM":
        ...
        await update.message.reply_text(
            f"✅ شماره حساب شما با موفقیت به `{text}` تغییر یافت."
        )
        context.user_data["state"] = None
        return True
    ...
    if state == "WAITING_FOR_FACTORY_QTY":
        if not text.isdigit() or int(text) <= 0:
            await update.message.reply_text(
                "❌ لطفاً یک عدد معتبر و بزرگتر از صفر وارد کنید."
            )
            return True
    ...
    if state == "WAITING_FOR_CONTRA_QTY":
        ...
        await update.message.reply_text(
            f"🛒 تعداد {qty} عدد **{item_name}** به سبد قاچاق اضافه شد."
        )
        context.user_data["state"] = None
        return True
...
async def show_sell_menu(
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "👕 فروش لباس (هر عدد ۴۰)",
                callback_data=f"sell_clothes:{user_id}",
            )
        ],
        [
            InlineKeyboardButton(
                "🦴 فروش غذا (هر عدد ۸۰)", callback_data=f"sell_food:{user_id}"
            )
        ],
        [
            InlineKeyboardButton(
                "🎾 فروش توپ (هر عدد ۲۵)", callback_data=f"sell_toy:{user_id}"
            )
        ],
        [
            InlineKeyboardButton(
                "🏠 فروش لانه (هر عدد ۸۰۰)",
                callback_data=f"sell_house:{user_id}",
            )
        ],
        [
            InlineKeyboardButton(
                "🚬 فروش سیگار قاچاق (هر عدد ۱,۲۰۰)",
                callback_data=f"sell_cig:{user_id}",
            )
        ],
        [
            InlineKeyboardButton(
                "💎 فروش الماس سیاه (هر عدد ۵,۰۰۰)",
                callback_data=f"sell_diamond:{user_id}",
            )
        ],
    ]
...
async def sell_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prices = {
        "sell_clothes": {
            "field": "inventory_clothes",
            "price": 40,
            "name": "لباس هاپویی",
        },
        "sell_food": {
            "field": "inventory_food",
            "price": 80,
            "name": "غذای ویژه",
        },
        "sell_toy": {
    ...
    if data in prices:
        ...
        if current_count <= 0:
            await query.message.reply_text(
                f"❌ شما هیچ عددی از **{item['name']}** در انبار ندارید که بفروشید!"
            )
            return
...
async def city_status(
    ...
    total_hops = (
        safe_db_call("get_group_total_hops", chat_id)
        or safe_db_call("get_total_hops", chat_id)
        or 0
    )
    total_dogs = (
        safe_db_call("get_group_total_dogs", chat_id)
        or safe_db_call("get_total_dogs", chat_id)
        or 0
    )
    current_level = safe_db_call("get_city_level", chat_id) or 1

    next_req = CITY_LEVEL_REQUIREMENTS.get(
        current_level, CITY_LEVEL_REQUIREMENTS[5]
    )

    if (
        treasury >= next_req["treasury"]
        and total_hops >= next_req["hops"]
        and total_dogs >= next_req["dogs"]
        and current_level < 5
    ):
        current_level += 1
        ...
        await update.message.reply_text(
            f"🎉 **تبریک! شهر هاپویی شما به سطح {current_level} ارتقا یافت!** 🎉",
            parse_mode="Markdown",
        )
        next_req = CITY_LEVEL_REQUIREMENTS.get(
            current_level, CITY_LEVEL_REQUIREMENTS[5]
    ...
    next_level_text = (
        f"سطح {current_level + 1}" if current_level < 5 else "حداکثر سطح"
    )
...
async def donate_city(
    ...
    if not args or not args[0].isdigit():
        await update.message.reply_text(
            "⚠️ لطفاً مبلغ اهدا را به عدد مشخص کنید.\nمثال: `اهدا 1000`",
            parse_mode="Markdown",
        )
        return
    ...
    if amount <= 0:
        await update.message.reply_text(
            "❌ مبلغ اهدا باید بیشتر از 0 باشد."
        )
        return
    ...
    if user_points < amount:
        await update.message.reply_text(
            "❌ شما سکه کافی در کیف پول خود ندارید!"
        )
        return
    ...
    await update.message.reply_text(
        f"✨ با موفقیت مبلغ **{format_balance(amount)}** به خزانه شهر اهدا شد! 🏛",
        parse_mode="Markdown",
    )


async def handle_factory(
    update: Update, context: ContextTypes.DEFAULT_TYPE, user=None
):
    return await show_factory(update, context)


async def handle_smuggle(
    update: Update, context: ContextTypes.DEFAULT_TYPE, user=None
):
