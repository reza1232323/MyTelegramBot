...
# تابع نمایش پیام عضویت اجباری
async def show_join_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = []
    for i, channel_id in enumerate(config.REQUIRED_CHANNELS):
        try:
            chat = await context.bot.get_chat(channel_id)
            channel_username = chat.username if chat.username else f"کانال {i+1}"
            keyboard.append([InlineKeyboardButton(f"📢 عضویت در {channel_username}", url=f"https://t.me/{channel_username}")])
...
async def referral_section(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # بررسی عضویت
    if not await check_membership(user_id, context):
        await show_join_message(query, context)
        return
...
# هندلر متن (برای پیام‌های معمولی)
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text == "/start":
        await start(update, context)
    else:
...
def main():
    ...
    # اجرای ربات
    print("🤖 ربات خفن شروع به کار کرد!")
    print(f"✅ برای شروع به @{(application.bot.get_me()).username} پیام دهید")
    application.run_polling(allowed_updates=Update.ALL_TYPES)
